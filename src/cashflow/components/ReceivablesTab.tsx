...
export function ReceivablesTab() {
  ...
  const explainCash = () => setDrill({
    ...
    title: 'AR Collected Till Date', sub: 'Every customer payment recorded in Striven, all months to date: money actually in the door',
        ...
        <>
          <div className="kpi-r-strip" data-guide-anchor="ar-kpis">
            ...
            {/* ── "AR COLLECTED TILL DATE", renamed from "Cash Received" ───────
                ...
                which adds to the same $504,413 across Mar–Sep — not the period
                on screen. "Cash Received" read as a flow and sat beside four
                other tiles that are balances, so the one running total on the
                strip was the one figure whose scope the label did not state.
                The new name says both halves: what it counts, and that it
                counts everything up to today.

                NOT RENAMED WITH IT: the "Cash Received by Month" chart below
                and the glossary entry of that name. Those describe the PER
                MONTH figure, which is a different thing from a running total,
                and the glossary headword is referenced by `guide="Cash
                Received"` — renaming it would point that link at nothing. */}
            <KpiR ico="cash" tint="#059669" label="AR Collected Till Date" value={payments.total} format={formatCurrency}
