                  ...
                  <td>
                    {/PO CREATED/.test(l.result)
                      ...
                      : /DRY-RUN/.test(l.result)
                        ? <span className="pill-tag tag-info">Dry-run: PO would be created</span>
...
export function AutomationTab() {
  ...
  const testLive = () => {
    if (!soId.trim()) return;
    if (window.confirm(`This will REALLY create a Purchase Order in Striven for SO ${soId} (DEMO gate still applies). Continue?`)) {
    ...
    <div className="exec-deck" style={{ padding: '4px 2px' }}>
      ...
      <div className="kpi-r-strip" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <KpiR ico="trend" tint={status?.mode === 'live' ? '#16A34A' : '#D97706'} label="Mode" value={status?.mode === 'live' ? 1 : 0}
          ...
          deltaText={status?.mode === 'live' ? 'POs are really created' : 'plan only · nothing is created'}
          foot="default from AUTO_PO_MODE" />
        <KpiR ico="shield" tint="#2563EB" label="Pilot Gate" value={status?.demoOnly === false ? 0 : 1}
          format={(n) => (n ? 'DEMO ONLY' : 'ALL ORDERS')}
          deltaText={status?.demoOnly === false ? 'every order is processed' : 'DEMO/test orders only'}
          foot="real patients stay untouched" />
        <KpiR ico="clock" tint="#7C3AED" label="Checkpoint" value={status?.checkpoint ?? 0}
          format={(n) => (n ? `SO ${Math.round(n)}` : '—')}
          deltaText="orders before this are never touched" foot="polling continues from here" />
        <KpiR ico="clip" tint="#0D9488" label="Runs Logged" value={status?.log?.length ?? 0}
          deltaText={`${status?.processedCount ?? 0} SOs processed (live)`} foot="last 20 runs listed below" />
      </div>

      <div className="exec-grid12">
        <div className="section chart-card g12-5">
          <div className="section-head"><div><h2 className="section-title">Test a Sales Order</h2><div className="section-sub">Enter an SO id and preview the run · SO-315 = DEMO Hidow</div></div></div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button className="ac-review" style={{ marginLeft: 0 }} disabled={!!busy || !soId.trim()} onClick={testLive}>
              {busy === 'live' ? 'Creating…' : '⚡ Live: create PO'}
            </button>
          </div>
          <div className="muted-note" style={{ marginTop: 12 }}>
            Dry-run creates nothing — it shows the exact plan (vendor, qty, drop-ship). The DEMO gate applies to Live as well.
          </div>
        </div>

        <div className="section chart-card g12-3">
          <div className="section-head"><div><h2 className="section-title">Poll Now</h2><div className="section-sub">Exactly what the cron will run</div></div></div>
          <div className="card-body" style={{ justifyContent: 'flex-start' }}>
            <button className="btn ghost" disabled={!!busy} onClick={pollNow}>{busy === 'poll' ? 'Polling…' : '⟳ Check for new SOs'}</button>
            <div className="muted-note" style={{ marginTop: 10 }}>Processes Sales Orders created after the checkpoint (max 3 per run).</div>
          </div>
        </div>

        <div className="section chart-card g12-4">
          <div className="section-head"><div><h2 className="section-title">How it works</h2><div className="section-sub">3 steps</div></div></div>
          <div className="ins-list">
            <div className="ins-item"><span className="ins-dot" style={{ background: 'rgba(37,99,235,0.10)', color: '#2563EB' }}>1</span><span>A new SO arrives (poll/test) → <b>DEMO gate</b> check</span></div>
            <div className="ins-item"><span className="ins-dot" style={{ background: 'rgba(124,58,237,0.10)', color: '#7C3AED' }}>2</span><span>Each item's <b>latest previous PO</b> supplies the vendor and terms</span></div>
            <div className="ins-item"><span className="ins-dot" style={{ background: 'rgba(22,163,74,0.12)', color: '#16A34A' }}>3</span><span>The PO is raised — <b>drop-shipped to the current customer</b>, titled with the SO number</span></div>
          </div>
        </div>
      </div>
