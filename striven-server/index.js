// SMR ⇄ Striven backend.
// Holds the Striven API credentials (never exposed to the browser), manages the
// OAuth2 client-credentials token, and serves the dashboard a small, shaped API:
//   GET /api/health   – liveness (no Striven call)
//   GET /api/status   – connection status + company name
//   GET /api/ar       – accounts receivable: open invoices + aging
//   GET /api/ap       – accounts payable: open bills + aging
//   GET /api/accounts – chart of accounts (bank / AR / AP / etc.)
//   GET /api/pl       – rough P&L (income vs. expense) — approximate, see note
//
// Dependency-free: Node built-ins only (http, fs, global fetch). Run: `npm start`.
...
// HIPAA: Sports Med Recovery's Striven "customers" are patients, so names /
// notes / addresses / custom fields are PHI. With MASK_PHI on (default), the
// backend never sends raw patient identifiers to the browser — it emits only
// masked initials + internal ids and drops free-text/clinical fields. Flip to
// 'false' ONLY behind proper auth on a HIPAA-covered host with a Striven BAA.
const MASK_PHI = (process.env.MASK_PHI ?? 'true') !== 'false';
// Optional access gate for public/tunnel previews. When ACCESS_PASSWORD is set,
// every /api call (except /api/health and /api/login) requires a session cookie
// obtained by POSTing the password to /api/login. Protects the PHI-bearing API,
// not just the UI. Leave empty for un-gated local dev.
const ACCESS_PASSWORD = process.env.ACCESS_PASSWORD || '';
const SESSION_TOKEN = ACCESS_PASSWORD ? crypto.createHash('sha256').update(`${ACCESS_PASSWORD}::smr-session`).digest('hex') : '';
...
async function getToken(force = false) {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) throw new Error('Striven credentials not configured (set STRIVEN_CLIENT_ID / STRIVEN_CLIENT_SECRET in striven-server/.env)');
...
async function striven(method, endpoint, jsonBody) {
  ...
  if (res.status === 401) { await getToken(true); res = await doCall(); } // token expired → refresh once
  // Striven Standard plan = 100 calls/min. On 429, wait Retry-After and retry.
  for (let attempt = 0; res.status === 429 && attempt < 3; attempt++) {
    const waitS = Number(res.headers.get('retry-after')) || 5;
    console.warn(`[striven] 429 rate-limited on ${endpoint}; waiting ${waitS}s`);
...
// Page through a POST .../search endpoint until all rows are read (capped).
...
// ---- cache --------------------------------------------------------------
// Striven Standard = 100 calls/min, and every tab re-fetches. So each base
// dataset is fetched ONCE per TTL and every endpoint computes from the cached
// set. Concurrent callers share one in-flight promise (no thundering herd).
const CACHE_TTL = Number(process.env.CACHE_TTL_MS || 300_000); // 5 min
...
function cached(key, fn) {
  ...
  const p = Promise.resolve().then(fn)
    ...
    .catch((e) => {
      ...
      if (hit) return hit.value; // serve stale on error (e.g. transient 429) rather than fail the tab
      throw e;
    });
  _inflight.set(key, p);
  return p;
}
// Base datasets (each = a few Striven calls, cached). Everything derives from these.
...
// Canceled / voided / failed transactions must not count toward any financial
// total (a voided bill isn't money owed; a canceled PO isn't spend).
...
// ---- PHI masking --------------------------------------------------------
// Reduce a patient name to initials ("Marlynn Fett" -> "M.F.") when masking is
// on. Vendors (businesses) are NOT patients — pass mask=false for those.
...
// SO/PO reference numbers are often patient-derived ("BLopez"). When masking,
// show a neutral id-based ref instead of the raw (name-leaking) number.
...
async function getAR() {
  ...
  const invoices = inv.map((r) => ({
    ...
    // memo dropped: free-text may contain PHI (e.g. "Patient PO#…" / clinical notes)
  }));
...
// Rough P&L: revenue = invoiced this calendar year; expenses = billed this year.
// Approximate (not an accrual/GAAP statement) — for exact figures use a Striven
// Report Builder API-key report.
...
// Sales Orders. Search rows carry no order total (only detail does), so we
// report counts + status mix + a masked recent list, not a summed value.
...
// Purchase Orders. Vendors are businesses (not PHI), so vendor names stay;
// dropShipCustomer (a patient) would be PHI but isn't in the search row.
...
// ---- extended extraction: patients, vendors, catalog, order detail, trends ----
const CUST_STATUS = { 1: 'Prospect', 2: 'Active', 3: 'Deleted', 4: 'Lost' };

// Patients (Striven customers). Names masked to initials; ref is a neutral id.
...
// Vendors are businesses (not PHI) — names kept.
async function getVendors() {
  const rows = await allVendors();
  const vendors = rows.map((r) => ({
    id: r.id, name: r.name ?? '', number: r.number ?? '',
    status: r.status?.name ?? String(r.status ?? ''),
    phone: r.phoneNumber ?? '', terms: r.paymentTerms?.name ?? '',
  }));
  return { count: vendors.length, vendors };
}

// Item catalog (services/products). The catalog itself isn't tied to a patient,
// so names/prices are not PHI.
async function getItems() {
  const rows = await allItems();
  const items = rows.map((r) => ({
    id: r.id, name: r.name ?? '', number: r.itemNumber ?? '',
    type: r.itemType?.name ?? '', description: r.description ?? '',
    price: Number(r.price ?? 0), cost: Number(r.cost ?? 0), active: r.active ?? false,
  }));
...
// PO detail — what was purchased from a vendor (safe: no patient involved).
async function getPODetail(id) {
  ...
  return {
    ...
    status: r.status?.name ?? '', vendorStatus: r.vendorStatus?.name ?? '',
    type: nm(r.type), title: r.title ?? '',
    poDate: r.poDate ?? r.dateCreated ?? null, promiseDate: r.promiseDate ?? null,
    // Who raised / handled it — these are internal staff & vendor contacts, not patients.
    requestedBy: nm(r.requestedBy), contact: nm(r.contact),
    createdBy: nm(r.createdBy), createdDate: r.dateCreated ?? null,
    approvedDate: r.approvedDate ?? null, reviewedDate: r.reviewedDate ?? null,
    acceptedBy: nm(r.acceptedByContact), lastUpdatedBy: nm(r.lastUpdatedBy),
    paymentTerm: nm(r.paymentTerm), account: nm(r.apglAccount),
    // A drop-ship destination is a patient → mask it.
...
// SO detail — a patient's order. With masking on, withhold the line-item detail
// (item + patient = treatment = PHI); expose only totals/counts.
...
// Monthly revenue (invoices) vs expenses (bills) for trend charts. Aggregate,
// non-PHI.
async function getTrends() {
  ...
  const months = {};
  const bump = (dateStr, key, amt) => {
    if (!dateStr) return;
    const m = String(dateStr).slice(0, 7);
    months[m] = months[m] || { month: m, revenue: 0, expenses: 0 };
    months[m][key] += amt;
  };
  ...
  const series = Object.values(months)
    .map((m) => ({ ...m, revenue: round2(m.revenue), expenses: round2(m.expenses), net: round2(m.revenue - m.expenses) }))
    .sort((a, b) => a.month.localeCompare(b.month))
    .slice(-12);
  return { series };
}

// Payments received (AR cash-in). Mask patient names.
...
// Bill payments (AP cash-out via credit card). Vendors are not PHI.
...
// Tasks. Titles can embed patient names → masked to a ref by default.
...
// Projects. Names can embed patient names → masked to a ref by default.
...
// Dynamic routes: /api/po/:id and /api/so/:id
const DYNAMIC = [
...
const server = http.createServer(async (req, res) => {
  ...
  if (ACCESS_PASSWORD) {
    ...
    if (url.pathname !== '/api/health' && getCookie(req, 'smr_session') !== SESSION_TOKEN) {
  ...
  if (!handler) {
    for (const d of DYNAMIC) {
      ...
      if (m) { handler = () => d.handler(m); break; }
    }
  }
  if (!handler) { res.writeHead(404, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: 'not found' })); }
  try {
...
server.listen(PORT, () => {
  console.log(`SMR ⇄ Striven backend on http://localhost:${PORT}`);
  console.log(`Credentials ${CLIENT_ID && CLIENT_SECRET ? 'loaded' : 'MISSING — set striven-server/.env'}`);
});
